...
//Mention and respond
  ...
  if ([`<@${client.user.id}>`, `<@!${client.user.id}>`].includes(message.content) && !args[0]) {
    let embed = new MessageEmbed()
      .addField("Prefijo del bot", `El prefijo en este servidor es \`${prefixs[0]}\``)
      .addField("Invitar al bot", `**[Click aquí]()**`)
      .addField("Comunidad & Servidor Oficial:", `**[Click Here!](https://discord.gg/FT6ZV5AGVf)**`)
      .setColor(color)
...
//Blacklist and Suspend
  ...
  if (message.content.startsWith(guild.prefix.toLowerCase()) && guild.blacklist == true) return message.channel.send("> :x: **This server has been blacklisted. [Appeal Here](https://dyno.gg/form/891f4d86)**");



  if (user && user.blacklist == true && message.content.toLowerCase().startsWith(guild.prefix)) return message.channel.send("> :x: **You have been blacklisted.[Appeal Here](https://dyno.gg/form/891f4d86)**");
...
async function spawnPokemon(message, client) {
  ...
  await get(options).then(async t => {
    ...
    let embed = new MessageEmbed()
      .setAuthor("Un pokémon salvaje ha aparecido!",`${poke.url}`)
      .setDescription(` Adivina el pokémon y escribe \`${guild.prefix}catch <nombre pokémon>\` para capturarlo!`)
    ...
    if (guild.incense == false) {
      time = 20000 - Math.floor(Math.random() * 30000)
    }
...
async function leveling(message, client) {
  ...
  let embed9 = new MessageEmbed()
    .setAuthor(`Felicidades ${message.author.username}!`)
    .setDescription(`Tu ${user.pokemons[selected].shiny ? "✨" : ""} ${capitalize(user.pokemons[user.selected].name)} ha subido de nivel al ${poke.level + 1}.`)
