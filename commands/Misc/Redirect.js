...
module.exports = {
    ...
    if(!nchannel) return message.channel.send(`> Correct usage: **${nguild.prefix || client.config.prefix}redirect <mentionchannel/id/name>**`);
      ...
      await nguild.save();
      return message.channel.send(`> ${config.yes} **PokeFree will now redirect all spawns to <#${nguild.spawnchannel}> To allow spawns in all channels again, type **\`${prefix}redirect disable.\``);
  }
}   